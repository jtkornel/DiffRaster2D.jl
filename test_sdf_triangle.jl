...
N=16
M=16
ts = [triangle([x*W/(N-1), y*H/(M-1)], [(x+1)*W/(N-1), y*H/(M-1)], [(x+0.5)*W/(N-1), (y+1)*H/(M-1)], color_shade([128.0f0, 128.0f0, 128.0f0])) for x in 0:(N-1) for y in 0:(M-1)]
...
function gradient_iteration(scne, d=1)
    ...
    ts = [triangle(scne.ts[ti].a .- d*gs.ts[ti].a, scne.ts[ti].b .- d*gs.ts[ti].b, scne.ts[ti].c .- d*gs.ts[ti].c, updated_color_shade(scne.ts[ti].csh, gs.ts[ti].csh, d)) for ti in 1:length(scne.ts)]
